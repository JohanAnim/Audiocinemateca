...
- **Solicitud Dinámica Múltiple en Inicio:** Actualizado `SplashActivity.kt` usando `RequestMultiplePermissions()` para solicitar activamente en Android 12+ (API 31+) y Android 13+ (API 33+) los permisos de dispositivos cercanos y notificaciones, garantizando la detección instantánea de dispositivos Chromecast y bocinas en la red local.
