...
- **Blindaje de Modo Offline Total y Transmisión Cast de Contenido Descargado ([PlayerFragment.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/presentation/player/PlayerFragment.kt), [ContentDetailFragment.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/presentation/contentdetail/ContentDetailFragment.kt) & [LocalCastProxyServer.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/util/LocalCastProxyServer.kt)):**
  - Al estar activado el Modo Offline (`offline_mode`), si el usuario intenta reproducir un contenido que no ha sido descargado previamente, se bloquea la reproducción mostrando un diálogo explicativo no cancelable y al pulsar "Aceptar" se cierra el reproductor.
  - En la pantalla de detalles y el reproductor, se ocultan y bloquean todas las funciones que requieren conexión a internet (comentarios, votaciones Me Gusta/No Me Gusta, clasificación por IA, sinopsis por IA y diálogos de calificación).
  - Corregida la transmisión a Google Cast de archivos descargados: `LocalCastProxyServer.buildCastUrl` ahora prioriza estrictamente las URIs de almacenamiento local (`content://`, `file://`), permitiendo que el servidor HTTP proxy embebido sirva el audio directamente al Chromecast sobre la red Wi-Fi local sin requerir conexión a internet externa.
- **Mensaje de Motivación al Compartir con IA ([AiSettingsScreen.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/presentation/settings/compose/AiSettingsScreen.kt), [GeminiRepository.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/data/repository/GeminiRepository.kt) & [ShareUtils.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/util/ShareUtils.kt)):**
  - Añadido interruptor `Generar mensaje de motivación al compartir` en Ajustes -> Inteligencia Artificial (`ai_share_motivational_message`, desactivado por defecto).
  - Al estar activado, antes de abrir el selector de compartir nativo, Gemini (`generateMotivationalShareMessage`) redacta una recomendación entusiasta, cálida y sin spoilers sobre la obra seleccionada y le adjunta automáticamente el enlace oficial de Audiocinemateca. Si está desactivado o sin conexión, utiliza el formato predeterminado.
- **Control de Presencia en Línea en Ajustes de Comunidad ([CommunitySettingsScreen.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/presentation/settings/compose/CommunitySettingsScreen.kt), [SettingsViewModel.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/presentation/settings/compose/SettingsViewModel.kt), [MainActivity.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/MainActivity.kt) & [GlobalChatViewModel.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/presentation/community/GlobalChatViewModel.kt)):**
  - Añadido interruptor `Mostrar presencia en línea` en Ajustes -> Comunidad (activado por defecto).
  - Al desactivar la opción, se oculta inmediatamente al usuario de la lista de conectados (`setUserPresence(isOnline = false)` en Firestore y backend) y se detiene el heartbeat en tiempo real. Al volver a activarlo, se restablece la presencia automáticamente.
- **Botón de Lectura de Sinopsis con TTS en Detalle ([ContentDetailFragment.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/presentation/contentdetail/ContentDetailFragment.kt) & [fragment_content_detail.xml](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/res/layout/fragment_content_detail.xml)):**
  - Implementado botón reactivo `Leer sinopsis con TTS` ubicado inmediatamente debajo de la sinopsis del contenido.
  - Integrado con la utilidad nativa `TtsManager` (`speak(sinopsis, interrupt = true)`) respetando las preferencias de motor, voz, tono y velocidad del usuario.
  - Alterna dinámicamente su estado a `Detener lectura TTS` con icono `ic_close` durante la locución y se restablece automáticamente al finalizar (`onSpeechFinished`) o salir de la pantalla (`onPause` / `onDestroyView`).
- **Sinopsis de Capítulo con IA ([GeminiRepository.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/data/repository/GeminiRepository.kt) & [PlayerViewModel.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/presentation/player/PlayerViewModel.kt)):**
  - Implementado botón expandible `✨ Sinopsis (IA)` en los controles del reproductor ([exoplayer_custom_controls.xml](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/res/layout/exoplayer_custom_controls.xml)) con etiqueta unificada limpia, estado semántico nativo (`stateDescription = "Contraído" / "Expandido"`), delegate nativo de accesibilidad (`ViewCompat.setAccessibilityDelegate` con acciones "Expandir" / "Contraer" sin duplicados en TalkBack) y respuesta háptica.
  - Directiva de redacción cinematográfica profesional de 2 a 4 oraciones con estricta regla de **cero spoilers**.
  - Sistema de caché reactivo en memoria por clave de episodio para evitar peticiones duplicadas y auto-actualización inmediata al cambiar de capítulo.
- **Rediseño Compacto de la Barra de Comentarios ([exoplayer_custom_controls.xml](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/res/layout/exoplayer_custom_controls.xml)):**
  - Reducido el padding vertical y optimizados los tamaños tipográficos de la tarjeta de comentarios para dejar un espacio visual limpio y proporcionado.
- **Blindaje Total de Conexión en Google Cast ([PlayerService.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/presentation/player/PlayerService.kt), [LocalCastProxyServer.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/util/LocalCastProxyServer.kt) & [CastOptionsProvider.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/presentation/cast/CastOptionsProvider.kt)):**
  - Incorporados `WakeLock` y `WifiLock` durante las sesiones de Cast para evitar desconexiones en segundo plano o con la pantalla apagada.
  - Streaming HTTP sin timeouts artificiales (`readTimeout = 0`, `soTimeout = 0`) y auto-reconexión del SDK de Google Cast (`setResumeSavedSession(true)` y `setEnableReconnectionService(true)`).
  - Cierre completo del servicio al descartar el mini reproductor en reproducción local (`!isCastActive`), conservando la conexión exclusivamente durante sesiones de Cast activas.

## Hito: Notificación Multimedia Nativa (MediaStyle), Sincronización Cast y Persistencia de Conexión
- **Notificación Multimedia Nativa en Android ([PlayerService.kt](file:///E:/Johan/AndroidStudioProjects/Audiocinemateca/app/src/main/java/com/johang/audiocinemateca/presentation/player/PlayerService.kt)):**
  - Implementado `MediaStyleNotificationHelper.MediaStyle(session)` asociando el token nativo del `MediaSession`.
  - Integrados controles multimedia directos (Anterior, Reproducir/Pausar dinámico, Siguiente) con soporte para vista compacta (`setShowActionsInCompactView(0, 1, 2)`).
  - Añadido `setDeleteIntent` para descartar la notificación cuando la reproducción está pausada y configurado canal de notificación sin alertas intrusivas.
  - Integración total con el reproductor del sistema de Android (Quick Settings / Panel de Notificaciones en Android 8 a 17).
- **Sincronización Inteligente de Estado Play/Pause en Cast:**
  - Al conectar un dispositivo Google Cast/Smart TV mientras el reproductor está en pausa, se mantiene el estado pausado sin iniciar audio de forma involuntaria. Si estaba reproduciendo, la reproducción continúa de forma fluida e instantánea en el dispositivo remoto.
  - Sincronización bidireccional inmediata de botones e interfaces (`ACTION_UPDATE_PLAY_PAUSE_BUTTON`).
- **Aislamiento y Supresión de Notificaciones Duplicadas en Cast:**
  - Cuando la reproducción está activa en Google Cast, `ensureForegroundNotification()` retira la notificación del teléfono (`stopForeground(STOP_FOREGROUND_REMOVE)`), dejando el control exclusivo a la notificación oficial de Google Cast SDK.
  - Al desconectarse de Cast y regresar a la reproducción local en el teléfono, la notificación `MediaStyle` se restaura de forma automática e inmediata.
- **Preservación de la Conexión de Cast al Cerrar el Mini Reproductor:**
  - Al presionar el botón de cerrar (`X`) del mini reproductor, se detiene y limpia de forma segura la pista en el dispositivo remoto y local, se oculta la interfaz y se remueve la notificación.
  - Si la sesión de Cast está activa, no se destruye el servicio ni se libera el `CastPlayer`, permitiendo reproducir nuevas obras posteriormente sin requerir reconectar la bocina/TV.
